...
int ZStruct::get_frozen(int nreact1)
{
  ...
  for (int i=1;i<=nreact1;i++)
  {
    ...
    if (stat(filename.c_str(), &sts) != -1)
    {
      ...
      while (!frzfile.eof())
      {
        success=static_cast<bool>(getline(frzfile, line));
...
int ZStruct::read_string(int wfile, double* energies, int natoms0, double** xyz)
{
  ...
  string line;
  bool success=true;
  success=static_cast<bool>(getline(strfile, line));
...
//    exit(1);
  ...
  while(!strfile.eof())
  {
    if (!first) success = static_cast<bool>(getline(strfile, line));
    else first = 0;
...
int ZStruct::get_limits()
{
  ...
  while (!limfile.eof())
  {
    success=static_cast<bool>(getline(limfile, line));
    tok_line = StringTools::tokenize(line, " \t");
...
int ZStruct::read_shuttles()
{
  ...
  while (!shfile.eof())
  {
    success=static_cast<bool>(getline(shfile, line));
    tok_line = StringTools::tokenize(line, " \t");
